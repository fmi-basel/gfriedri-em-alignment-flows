...
# <<< conda initialize <<<
mamba run -p "$root_dir/infrastructure/miniforge3/envs/nf" nextflow run "$root_dir/gfriedri-em-alignment-flows/s03_fine-alignment/workflow.nf" --config $(pwd)/fine-align.yaml --rm_config $(pwd)/relax-meshes.yaml -profile slurm -with-report -resume
