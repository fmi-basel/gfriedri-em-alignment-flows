#!/bin/bash
#SBATCH --job-name=Tile-Stitching
...
# <<< conda initialize <<<
mamba run -p "$root_dir/infrastructure/miniforge3/envs/nf" nextflow run "$root_dir/gfriedri-em-alignment-flows/s01_tile-stitching/workflow.nf" --config $(pwd)/tile_stitching_config.yaml -profile slurm -with-report -resume -disable-jobs-cancellation
